...
class _AugDrift(AbstractTerm):
    ...
    def vf(self, t: Scalar, y: PyTree, args) -> PyTree:
        ...
        context = args[0]
        u = args[1]
        #print("context", context, "u", u.evaluate(t), "y", y) 
        aug_y = y if context is None else jnp.concatenate([y, context.evaluate(t), u.evaluate(t)], axis=-1)

        drift1 = self.drift1.vf(t, aug_y, context)
        drift2 = self.drift2.vf(t, y, u)

        drift = jtu.tree_map(operator.sub, drift1, drift2)
        diffusion = self.diffusion.vf(t, y, context)
